...
| 2026-07-13 | **Fase 8 — Internal OS (núcleo)** (branch): panel privado de plataforma **totalmente separado** (scope `internal`, guard deny-by-default, layout dark propio; business/creator/afiliado → 403) + **doble libro financiero** `@partnera/platform-finance` Revenue(A)/Vault(B) append-only **sin mezclar** + home operativo + páginas Ingresos/Vault/Empresas/Salud/Alertas. Resto de páginas **andamiadas** (honesto). **20 paquetes, 239 tests.** Dinero **simulado**; activación física (Postgres/deploy/Shopify) sigue pendiente y **separada**. Reporte: [docs/internal-os/PHASE8_REPORT.md](docs/internal-os/PHASE8_REPORT.md). D-327. |
| 2026-07-13 | **Fase 6+7 — Live Activation & Certification** (branch): adaptadores **reales** de deploy implementados (`PgSqlClient` write-behind sobre `pg`, `RealShopifyApi`, `RealSessionTokenVerifier`, `server-prod`, runner de contract contra Postgres real) — fuera del build verde porque `pg`/Postgres/deploy/Shopify **no existen en esta máquina** (install scripts bloqueados). **230 tests locales verdes.** Verdicto honesto: **PARTNERA NOT INSTALLED** — activación en vivo pendiente de recursos+autorización de Brian. Reportes: [PHASE6_LIVE_ACTIVATION_REPORT](docs/shopify-pilot/PHASE6_LIVE_ACTIVATION_REPORT.md) · [PHASE7_FINAL_CERTIFICATION](docs/shopify-pilot/PHASE7_FINAL_CERTIFICATION.md). D-326. |
